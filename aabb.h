...
typedef struct struct_aabb {
  ...
  bool (*overlaps)(struct struct_aabb *self, struct struct_aabb *aabb_2, vec2 *intersection);

  /// Tests for a point within the AABB
  /// \param self
  /// \param p
  /// \return cTRUE when point is inside AABB, otherwise cFALSE.
  bool (*containsPoint)(struct struct_aabb *self, vec2 p);
} aabb;
...
bool cAABBOverlap(aabb *self, aabb *aabb, vec2 *intersection);
